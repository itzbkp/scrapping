...
(async function () {
    ...
    if (!fs.existsSync("LinkedIn.csv"))
        await axios.get(
            'https://datacleaner.whoisvisiting.com/LinkedInQueue.ashx', {
            params: {
                count: 1000
            },
            headers: {
                                ...
                                try {
                                    ...
                                    for (i = start; i < inputURLs.length; i++) {
                                        ...
                                        if (htmlContent.length < 1000) {
                                            ...
                                            await axios.post(
                                                'https://datacleaner.whoisvisiting.com/LinkedInDataFail.ashx',
                                                {
                                                    URLID: inputIds[i],
                                                    URL: inputURLs[i]
                                                }, {
                                            ...
                                            const extractedData = await page.evaluate((Index, URL, URLID, xpaths, headers) => {
                                                ...
                                                let PrimaryLocation = document.evaluate('//span[@class = "locations__primary-label"]', document, null, XPathResult.STRING_TYPE).stringValue.trim();
                                                if (PrimaryLocation)
                                                    PrimaryLocation = JSON.stringify({
                                                        Address1: document.evaluate('//span[@class = "locations__primary-label"]//parent::li//p[1]', document, null, XPathResult.STRING_TYPE).stringValue.trim(),
                                                        Address2: document.evaluate('//span[@class = "locations__primary-label"]//parent::li//p[2]', document, null, XPathResult.STRING_TYPE).stringValue.trim(),
                                                        Map: document.evaluate('//span[@class = "locations__primary-label"]//parent::li//a/@href', document, null, XPathResult.STRING_TYPE).stringValue.trim()
                                                    });
                                                let SecondaryLocations = document.evaluate('count(//li[@class = "locations__location"][not(span)])', document, null, XPathResult.NUMBER_TYPE).numberValue;
                                                if (SecondaryLocations) {
                                                    const locations = [];
                                                    const node = document.evaluate('//li[@class = "locations__location"][not(span)]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE);
                                                    for (let i = 0; i < node.snapshotLength; i++) {
                                                        const address1 = node.snapshotItem(i).children[0];
                                                        const address2 = node.snapshotItem(i).children[1];
                                                        const map = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "A");
                                                        locations.push({
                                                            Address1: address1 ? address1.innerText : "",
                                                            Address2: address2 ? address2.innerText : "",
                                                            Map: map ? map.href : ""
                                                        });
                                                    }
                                                    SecondaryLocations = JSON.stringify(locations);
                                                }
                                                else
                                                    SecondaryLocations = "";
                                                let Employees = document.evaluate('count(//ul[@class = "employees__list"]//li)', document, null, XPathResult.NUMBER_TYPE).numberValue;
                                                if (Employees) {
                                                    const employees = [];
                                                    const node = document.evaluate('//ul[@class = "employees__list"]//li', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE);
                                                    for (let i = 0; i < node.snapshotLength; i++) {
                                                        const detailsNode = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "DIV");
                                                        const profile = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "A");
                                                        const image = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "IMG");
                                                        const name = Array.from(detailsNode.children).find(item => item.nodeName === "H3");
                                                        const description = Array.from(detailsNode.children).find(item => item.nodeName === "H4");
                                                        employees.push({
                                                            Profile: profile ? profile.href : "",
                                                            Image: image ? image.src || image.getAttribute("data-delayed-url") : "",
                                                            Name: name ? name.innerText.replace(/\\n/gm, "").trim() : "",
                                                            Description: description ? description.innerText.replace(/\\n/gm, "").trim() : ""
                                                        })
                                                    }
                                                    ...
                                                    Employees = "";
                                                let SimilarPages = document.evaluate('count(//section[contains(@class, "similar-pages")]//li)', document, null, XPathResult.NUMBER_TYPE).numberValue;
                                                if (SimilarPages) {
                                                    const pages = [];
                                                    const node = document.evaluate('//section[contains(@class, "similar-pages")]//li', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE);
                                                    for (let i = 0; i < node.snapshotLength; i++) {
                                                        const detailsNode = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "DIV");
                                                        const profile = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "A");
                                                        const logo = Array.from(node.snapshotItem(i).children).find(item => item.nodeName === "IMG");
                                                        const company = Array.from(detailsNode.children).find(item => item.nodeName === "H3");
                                                        const industry = Array.from(detailsNode.children).find(item => item.nodeName === "H4");
                                                        pages.push({
                                                            Profile: profile ? profile.href : "",
                                                            Logo: logo ? logo.src : "",
                                                            Company: company ? company.innerText.replace(/\\n/gm, "").trim() : "",
                                                            Industry: industry ? industry.innerText.replace(/\\n/gm, "").trim() : ""
                                                        })
                                                    }
                                                    SimilarPages = JSON.stringify(pages);
                                                }
                                                else
                                                    SimilarPages = "";
                                                return { ...extractedData, PrimaryLocation, SecondaryLocations, Employees, SimilarPages };
                                            }, extractorIndex, inputURLs[i], inputIds[i], xpaths, headers);
                                            consoleLogger.log("Step - 8");
                                            writer.write(extractedData);
                                            await axios.post(
                                                'https://datacleaner.whoisvisiting.com/linkedindata.ashx',
                                                {
                                                    ...extractedData,
                                                    PrimaryLocation: extractedData.PrimaryLocation ? JSON.parse(extractedData.PrimaryLocation) : undefined,
                                                    SecondaryLocations: extractedData.SecondaryLocations ? JSON.parse(extractedData.SecondaryLocations) : [],
                                                    Employees: extractedData.Employees ? JSON.parse(extractedData.Employees) : [],
                                                    SimilarPages: extractedData.SimilarPages ? JSON.parse(extractedData.SimilarPages) : []
                                                }, {
                                                headers: {
                                                    Authorization: 'BFCC45E1-DAB4-4303-A987-F1E50C6350E2'
                                                }
